...
function zoo_enqueue_wallet_scripts() {
    ...
    if (function_exists('is_checkout') && is_checkout() && function_exists('WC') && WC()) {
        ...
        $zoo_mint      = !empty($zoo_settings['zoo_mint']) ? $zoo_settings['zoo_mint'] : '';
        $decimals      = isset($zoo_settings['decimals']) ? $zoo_settings['decimals'] : '9';
    ...
    class WC_Zoo_Token_Gateway extends WC_Payment_Gateway {
        ...
        public function init_form_fields() {
            $this->form_fields = [
                ...
                'shop_wallet' => [
                    'title' => 'Shop Solana wallet',
                    'type' => 'text',
                    'description' => 'Your Solana address to receive payments.',
                    'default' => ''
                ],
                'zoo_mint' => [
                    'title' => 'ZOO token mint address',
                    'type' => 'text',
                    'description' => 'SPL token mint address for ZOO (required for token transfers).',
                    'default' => ''
                ],
                'decimals' => [
                    'title' => 'ZOO token decimals',
                    'type' => 'number',
                    'description' => 'Token decimals (e.g. 9).',
                    'default' => '9'
                ],
            ];
        }
